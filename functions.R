...
cahPlotCol = function(x, main="Plot", xlab = paste(ncol(x$data), "features"), k = 3, lth = 7, lrect = -12, cex = 0.6, ylab = "height"){
  ...
  # Redefining labels
  x$order.lab = sub("ALBERTI_","A_", x$order.lab)
  x$order.lab = sub("COLONNA_","C_", x$order.lab)
  x$order.lab = sub("LORENZODEMEDICI_","L_", x$order.lab)
  x$order.lab = substring(x$order.lab, 1, 10)
  # Coloring them
  labels = vector(length = length(x$order.lab))
  labels[grep("A_", x$order.lab)] = "darkgreen"
  labels[grep("C_", x$order.lab)] = "red"
  labels[grep("L_", x$order.lab)] = "pink"
