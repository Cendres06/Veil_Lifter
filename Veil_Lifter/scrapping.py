...
def ressources_de_travail(liste_tags_language=['fr'], liste_tags_format = [], liste_tags_necessaire = [], liste_tags_interdits = [], liste_combinaisons_interdites = [],liste_pages_exclues = [],debug = False):
...
# 2. Garder les <> de lien dans les images (option à rajouter dans nettoyage_html) 
    ...
    #    return("Module_crédit_absent")
        module_credit_sale = module_credit_sale[0].split('\n')
...
def nettoyage_profondeur(chaine,lien=True):
    """Effectue un nettoyage en profondeur de l'HTML, détruisant tout HTMl sur son passage.
    Retourne une str.
    
    chaine -> str>"""
    if lien:
        requete = re.search('<(.|/s)*?>',chaine)
    else:
        requete = re.search('<[^a](.|/s)*?>',chaine)
    if requete:
        if requete.start()==0:
            new_chaine = nettoyage_profondeur(chaine[requete.end():],lien)
        elif requete.end()==len(chaine):
            new_chaine = nettoyage_profondeur(chaine[:requete.start()],lien)
        else:
            new_chaine = nettoyage_profondeur(chaine[:requete.start()]+chaine[requete.end():],lien)
    else:
        new_chaine = chaine
    return new_chaine

def champs_ok(champ):
    """Détermine si la chaine en paramètre est un nom de champ de crédit.
    Retourne un bool.
    
    champ -> str"""
    if re.search("[Tt]itre|([Aa]utr{0,1}(eur){0,1}(ice){0,1}s{0,1})|([EéeÉ]crit par)|[Dd]ate|([I|i]mage)|([Cc]rédit)|([Ll]icence)|([Ll]égal)|([Aa]ttribution)|([Cc]ode)|[Rr]emerciement|([Nn]ote)|([Cc]ommentaire)|([Rr]emarque)|[Cc]ontexte",champ):
        return True
    else:
        return False

def nettoyage_module_credit(module_credit_sale,debug = False):
    ...
    for element in module_credit_sale:
        ...
        if nettoyage_profondeur(element) !="":
            if debug:
                print("On étudie l'élément brut :",element)
            nettoyage = nettoyage_profondeur(element)
            if(nettoyage!=0):
                champs = nettoyage.split(':')
                if debug:
                    print("On étudie l'élément divisé :",champs)
                #On peut avoir effectué une séparation abusive
                if len(champs)>2:
                    valeur = ""
                    for string in champs[1:]:
                        valeur += string
                    champs = [champs[0] + valeur]
                    if debug:
                        print("Normalisation de l'élément brut :",champs)
                if len(champs)==1:
                    if debug:
                        print("élement unique ! On ajoute au dernier champ.")
                    if module_credit_propre[last_champ]=="Vide":
                        module_credit_propre[last_champ]=""
                    if re.search("([I|i]mage)|([Cc]rédit)|([Ll]icence)|([Ll]égal)|([Aa]ttribution)|([Cc]ode)",last_champ):
                        if debug:
                            print("L'élément concerne les images et licences, on garde les liens")
                        nettoyage = nettoyage_profondeur(element,False)
                        module_credit_propre[last_champ]+=nettoyage
                    else:
                        module_credit_propre[last_champ]+=champs[0]
                    if debug:
                        print("On ajoute au champs :",last_champ)
                elif champs[1]=="":
                    if debug:
                        print("L'élément est une fausse liste de deux.")
                    if champs_ok(champs[0]):
                        if debug:
                            print("Désigne un nouveau champ :",champs[0])
                        module_credit_propre[espace_en_trop(champs[0])] = "Vide"
                        last_champ = nettoyage_profondeur(espace_en_trop(champs[0]))
                    else:
                        if debug:
                            print("Valeur détectée :",champs[0])
                        if re.search("([I|i]mage)|([Cc]rédit)|([Ll]icence)|([Ll]égal)|([Aa]ttribution)|([Cc]ode)",last_champ):
                            if debug:
                                print("L'élément concerne les images et licences, on garde les liens")
                            nettoyage = nettoyage_profondeur(element,False)
                            ...
                            module_credit_propre[last_champ]+=champs[0]
                        print("On ajoute au champs :",last_champ)
                #Cas où la liste fait deux éléments et qu'elle est conforme
                else:
                    if debug:
                        print("On est dans le cas où element est de taille 2 pile")
                    #Cas du positif
                    if champs_ok(champs[0]):
                        if debug:
                            print("Oh, le premier élément ressemble à un champ !")
                        #Vrai positif
                        if len(module_credit_propre) ==0 or (last_champ in module_credit_propre.keys() and module_credit_propre[last_champ]!="Vide"):
                            if debug:
                                print("Je pense que vrai positif")
                            if re.search("([I|i]mage)|([Cc]rédit)|([Ll]icence)|([Ll]égal)|([Aa]ttribution)|([Cc]ode)",espace_en_trop(champs[0])):
                                if debug:
                                    print("L'élément concerne les images et licences, on garde les liens")
                                nettoyage = nettoyage_profondeur(element,False)
                                module_credit_propre[espace_en_trop(champs[0])]=espace_en_trop(nettoyage)
                            else:
                                module_credit_propre[espace_en_trop(champs[0])]=espace_en_trop(espace_en_trop(champs[1]))
                            if debug:
                                print("On a ajouté le champ :",espace_en_trop(champs[0]))
                            last_champ = nettoyage_profondeur(espace_en_trop(champs[0]))
                        #Faux positif
                        else:
                            if debug:
                                print("Je pense que faux positif")
                            valeur = ""
                            for string in champs[1:]:
                                valeur += string
                            champs = champs[0] + valeur
                            if debug:
                                print("On normalise :",champs)
                            if re.search("([I|i]mage)|([Cc]rédit)|([Ll]icence)|([Ll]égal)|([Aa]ttribution)|([Cc]ode)",last_champ):
                                if debug:
                                    print("L'élément concerne les images et licences, on garde les liens")
                                nettoyage = nettoyage_profondeur(element,False)
                                module_credit_propre[last_champ]+=espace_en_trop(nettoyage)
                            else:
                                module_credit_propre[last_champ]+=espace_en_trop(champs)
                            if debug:
                                print("On a ajouté le champ :",espace_en_trop(champs[0]))
                    #Négatif
                    else:
                        if debug:
                            print("On a affaire à deux valeurs orphelines.")
                        valeur = ""
                        for string in champs[1:]:
                            valeur += string
                        champs = champs[0] + valeur
                        ...
                        if re.search("([I|i]mage)|([Cc]rédit)|([Ll]icence)|([Ll]égal)|([Aa]ttribution)|([Cc]ode)",last_champ):
                            ...
                            module_credit_propre[last_champ]+=espace_en_trop(champs)
            if debug:
                print("Notre module de crédit ressemble à ça :")
                print(module_credit_propre)
                print("Last_champ créé :",last_champ)
...
def process_pour_credits_fr(id_page,debug=False):
    ...
    module_credit_propre = nettoyage_module_credit(module_credit_sale,debug)
...
def liste_fragments(liste_textes_ok,liste_pages_exclues = ['/fragment:scp-5764-1','/fragment:version-2','/fragment:version-3','/fragment:version-4','/fragment:version-5','/fragment:version-6'],nom_bd = 'fragments', liste_paramètres=[['fr'],[],['fragment'],[],[]],debug=False):
    """Cette fonction sert à récupérer les fragments des pages, c'est-à-dire ses différents composants stockés à une autre url, et à créer des bases de données.
...
def creation_bd_textes_fr(màj_liste = False):
    ...
    if màj_liste:
        ...
        liste_textes_ok = pd.read_csv('bases_de_donnees/liste_id_pages.csv', encoding="utf-8-sig")
    print("La liste des textes à travailler est faite.")
